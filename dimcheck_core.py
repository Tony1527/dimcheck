...
from dimcheck import DIMCHECK_MODULE_PATH
...
class Dimcheck:
    ...
    def is_dc(self,lhs: str,rhs: str, is_print: bool=True) -> bool:
        ...
        '''
        return self._dimchecker.is_dc(lhs,rhs, is_print)
    ...
    def _serialize(self) -> None:
        pickle_file = self._get_serialized_path(self.config_file)
        data = {}
        with open(pickle_file,"wb") as f:
            data["hash"] = self._cal_config_file_hash(self.config_file)
            data["env"] = self.env
...
class Dimchecker():
    ...
    def __init__(self,config_file="./si.json",env="si"):
        ...
        if unsolved_num!=0:
            for i in range(unsolved_num):
                expr,idx,unsolved_quants=dependence_queue.get()
                # print("{},{},{}".format(expr,idx,self._quant2str(unsolved_quants)))
    ...
    def is_dc(self, lhs, rhs, is_print: bool=True):
...
si=Dimcheck(config_file=_split_dir_file(DIMCHECK_MODULE_PATH)[0]+"/si.json",is_save=False)
